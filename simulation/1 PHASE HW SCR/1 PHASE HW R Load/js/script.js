...
var values = {
  ...
  GTP1: { name: "Gate Pulse" , sfire: 0, efire:0,
  type1: "Frequency: ", type2: "Fire Angle: ",
  unitfreq: " Hz",  
  unit: "\u00B0",  },
...
instance.bind("ready", () => {
  ...
  function createEnd() {
    ...
    var AM1T = instance.addEndpoint("AM1T", {
      anchor: ["Top"],
      isSource: true,
      isTarget: true,
      connector: "Flowchart",
    ...
    var AM1B = instance.addEndpoint("AM1B", {
      anchor: ["Bottom"],
      isSource: true,
      isTarget: true,
      connector: "Flowchart",
   ...
   if (correct_connections_flag) {
    ...
    $(
      '<div class="custom-menu"><div class="name-element"><div class="name-element"><div style="display: flex; justify-content: end; position: relative;top: -4px;height: 28px; margin-bottom: 2px ;"><button class="submit fa fa-times cross-btn" id="submit-' +
        ...
        ')"><div><label for="name-' +
        window.selectedControl +
        '">Name:</label><input type="text" class="set-input-name" maxlength="8" id="name-' +
        window.selectedControl +
        '" style="border-radius: 20px; padding:2px;"  placeholder="  ' +
        values[window.selectedControl]["name"] +
        '" onchange="changeName(' +
...
function dcSubmited(name) {
  ...
  if (a != "") {
    new_reading=true;
    values[name]["value"] = a;
    var ele = name + "-value";new_reading=true;
...
document.getElementById("check1").addEventListener("click", () => {
  if (wrong_connection.length == 0) {
    if (user_connection.length < 13) {
      alert("Please make all the connections");
    } else {
